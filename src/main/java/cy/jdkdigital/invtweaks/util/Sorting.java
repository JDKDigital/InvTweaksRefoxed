package cy.jdkdigital.invtweaks.util;
...
public class Sorting
{
    public static void executeSort(Player player, boolean isPlayerSort) {
        if (isPlayerSort) {
            ...
            if (cont != player.inventoryMenu) {
                InvTweaksConfig.ContOverride override = getOverrides(player, cont.getClass().getName());
    ...
    public static void playerSortClient(Player player, Map<String, InvTweaksConfig.Category> cats, InvTweaksConfig.Ruleset rules, IntList lockedSlots) {
        ...
        IntList stackIdxs = IntStream.range(0, inv.items.size())
        ...
        for (Map.Entry<String, InvTweaksConfig.Category> ent : cats.entrySet()) {
            ...
            Client.processCategoryClient(player, gatheredSlots, stackWs, ent.getValue(), toIt);
        }

        List<Slot> fallbackList =
                Stream.concat(Streams.stream(Optional.ofNullable(rules.catToInventorySlots("/OTHER"))).flatMap(List::stream), rules.fallbackInventoryRules().stream())
                        .mapToInt(v -> v)
                        .filter(idx -> Collections.binarySearch(lockedSlots, idx) < 0)
                        .distinct()
                        .mapToObj(indexToSlot::get)
        ...
        Client.processCategoryClient(player, gatheredSlots, stackWs, null, fallbackList.listIterator());
    }
    ...
    public static void playerSortServer(ServerPlayer player, Map<String, InvTweaksConfig.Category> cats, InvTweaksConfig.Ruleset rules, IntList lockedSlots) {
        ...
        List<ItemStack> stacks = Utils.condensed(() -> IntStream.range(0, inv.items.size())
                .filter(idx -> Collections.binarySearch(lockedSlots, idx) < 0)
                .mapToObj(inv.items::get)
                .filter(st -> !st.isEmpty()).iterator());
        ...
        for (Map.Entry<String, InvTweaksConfig.Category> ent : cats.entrySet()) {
            ...
            Streams.zip(specificRules.stream(), curStacks.stream(), Pair::of).forEach(pr -> inv.items.set(pr.getKey(), pr.getValue()));
        }
    ...
    public static void inventorySortClient(Player player, List<Slot> validSlots) {
        ...
        for (Equivalence.Wrapper<ItemStack> stackW : stackWs) {
            ...
            Client.clientPushToSlots(player, gatheredSlots.get(stackW).iterator(), toIt, displaced);
    ...
    static class Client
    {
        static void processCategoryClient(Player player, Map<Equivalence.Wrapper<ItemStack>, Set<Slot>> gatheredSlots, List<Equivalence.Wrapper<ItemStack>> stackWs, InvTweaksConfig.Category cat, ListIterator<Slot> toIt) {
            ...
            for (Equivalence.Wrapper<ItemStack> stackW : subStackWs) {
                if (cat == null || cat.checkStack(stackW.get()) >= 0) {
                    ...
                    Client.clientPushToSlots(player, fromIt, toIt, displaced);
                    for (Map.Entry<Slot, Slot> displacedPair : displaced.entrySet()) {
         ...
         */
        static boolean clientPushToSlots(Player player, Iterator<Slot> OriginIter, ListIterator<Slot> destinationIter, BiMap<Slot, Slot> displaced) {
            ...
            while (OriginIter.hasNext()) {
                ...
                pc.handleInventoryMouseClick(player.containerMenu.containerId, originSlot.index, 0, ClickType.PICKUP, player);
                ...
                while (destinationIter.hasNext()) {
                    ...
                    pc.handleInventoryMouseClick(player.containerMenu.containerId, destinationSlot.index, 0, ClickType.PICKUP, player);

                    // Didn't pick anything up -> done
    ...
    public static InvTweaksConfig.ContOverride getOverrides(String contClass) {
        return getOverrides(null, contClass);
    }

    public static InvTweaksConfig.ContOverride getOverrides(@Nullable Player player, String contClass) {
        var overrideMap = player != null ? InvTweaksConfig.getPlayerContOverrides(player) : InvTweaksConfig.getSelfCompiledContOverrides();
        InvTweaksConfig.ContOverride override = overrideMap.get(contClass);
        if (override == null) {
            var fuzzy = overrideMap.entrySet().stream().filter(entry -> {
                return Pattern.compile(entry.getKey()).matcher(contClass).matches();
            }).map(Map.Entry::getValue).toList();
            if (fuzzy.size() > 0) {
                override = fuzzy.get(0);
            }
        }
        return override;
    }
